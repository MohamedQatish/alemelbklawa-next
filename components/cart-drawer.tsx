...
export default function CartDrawer({
  ...
  useEffect(() => {
    if (open) {
      const savedPhone = localStorage.getItem("myOrdersPhone");
      if (savedPhone) {
        setCustomerPhone(savedPhone);
        setShowMyOrders(true);
        localStorage.removeItem("myOrdersPhone");
      }
    }
  }, [open]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          setCities(
            data.map((c: { city: string; price: number }) => ({
              name: c.city,
  ...
  const formatSelectedOptions = (
    selectedOptions: CartItemDisplay["selectedOptions"],
  ) => {
  ...
  async function handleSubmitOrder(e: React.FormEvent) {
    ...
    if (primaryCountry && cleanPhone.length !== primaryCountry.length) {
      toast.error(
        `رقم الهاتف لـ ${primaryCountry.name} يجب أن يتكون من ${primaryCountry.length} أرقام (بدون الصفر)`,
      );
    ...
    if (form.secondaryPhone) {
      ...
      const secCountry = countries.find(
        (c) => c.code === form.secondaryPhoneCode,
      );

      // فحص طول الرقم الثانوي (لو العميل كتبه)
      if (secCountry && cleanSec.length !== secCountry.length) {
        toast.error(
          `الرقم الثانوي لـ ${secCountry.name} يجب أن يتكون من ${secCountry.length} أرقام`,
        );
        return; // نوقف الإرسال فوراً
      }
      ...
      <div className="absolute top-0 right-0 bottom-0 w-full max-w-md overflow-y-auto border-r border-[var(--gold)]/20 bg-[var(--royal-red-dark)]">
        ...
        {orderSuccess ? (
          <div className="flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="mb-3 text-2xl font-bold text-[var(--cream)]">
              شكراً لاختيارك إيانا
            </h3>
            <p className="mb-6 text-[var(--gold)]/60">
            ...
            <div className="flex w-full flex-col gap-3">
              <button
                ...
                className="w-full rounded-xl bg-[var(--gold)]/20 py-4 font-bold text-[var(--gold)] transition-all hover:bg-[var(--gold)]/30"
              >
                عرض طلباتي
              </button>
              ...
              <button
                ...
                className="w-full rounded-xl border border-[var(--gold)]/20 py-4 font-bold text-[var(--cream)] transition-all hover:bg-[var(--gold)]/10"
              >
                العودة للرئيسية
              </button>
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="relative">
                ...
                <div className="flex w-full items-center rounded-xl border border-[var(--gold)]/20 bg-[var(--royal-red-light)]/60 focus-within:border-[var(--gold)]/50">
                  ...
                  <input
                    required
                    maxLength={
                      countries.find((c) => c.code === form.phoneCode)?.length
                    }
                    value={form.phone}
                    onChange={(e) => {
                          ...
                          .map((c) => (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <span className="w-6 text-sm font-bold text-[var(--gold)]">
                                  {c.iso}
                                </span>
                              ...
                              >
                                ...
                                <div
                                  className={`h-2 w-2 rounded-full ${form.phoneCode === c.code ? "bg-[var(--gold)]" : "bg-transparent"}`}
                                />
                              </div>
                            </div>
                          ))}
              ...
              <div className="relative">
                ...
                <div className="flex w-full items-center rounded-xl border border-[var(--gold)]/20 bg-[var(--royal-red-light)]/60 focus-within:border-[var(--gold)]/50">
                  ...
                  <input
                    maxLength={
                      countries.find((c) => c.code === form.secondaryPhoneCode)
                        ?.length
                    }
                ...
                {isSecondaryDropdownOpen && (
                  <>
                    <div
                      className="fixed inset-0 z-40"
                      onClick={() => setIsSecondaryDropdownOpen(false)}
                    />
                    <div className="absolute right-0 top-[100%] z-50 mt-2 w-[320px] max-w-full rounded-2xl border border-[var(--gold)]/20 bg-[#2a0a0f] p-4 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
                      <div className="mb-4 flex items-center rounded-xl bg-white px-3 py-2.5">
                        <Search className="h-5 w-5 text-gray-500" />
                          ...
                          .map((c) => (
                            ...
                            >
                              ...
                              >
                                ...
                                <div
                                  className={`h-2 w-2 rounded-full ${form.secondaryPhoneCode === c.code ? "bg-[var(--gold)]" : "bg-transparent"}`}
                                />
                              </div>
                            </div>
                          ))}
              ...
              <div>
                ...
                >
                  <option
                    value=""
                    className="bg-[var(--royal-red-dark)] text-[var(--gold)]"
                  >
                    اختر المدينة
                  </option>
                  {cities.map((city) => (
              ...
              <div className="rounded-xl border-2 border-[var(--gold)]/25 bg-[var(--royal-red-light)]/30 p-4">
                ...
                <div className="flex flex-col gap-2.5">
                  <label
                    className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 px-4 py-3.5 transition-all duration-200 ${form.paymentMethod === "card" ? "border-[var(--gold)] bg-[var(--gold)]/15 shadow-[0_0_12px_rgba(197,165,90,0.15)]" : "border-[var(--gold)]/15 hover:border-[var(--gold)]/30 hover:bg-[var(--gold)]/5"}`}
                  >
                    <input
                      type="radio"
                      name="payment"
                    ...
                    />
                    <div
                      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-all ${form.paymentMethod === "card" ? "border-[var(--gold)] bg-[var(--gold)]" : "border-[var(--gold)]/30"}`}
                    >
                      {form.paymentMethod === "card" && (
                        <div className="h-2 w-2 rounded-full bg-[var(--royal-red-dark)]" />
                      )}
                    </div>
                    <div className="flex flex-1 items-center gap-3">
                      <svg
                        className="h-6 w-6 text-[var(--gold)]/70"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={1.5}
                      ...
                      <div>
                        ...
                        <span className="block text-xs text-[var(--gold)]/50">
                          Visa / Mastercard
                        </span>
                      </div>
                    </div>
                  ...
                  <label
                    className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 px-4 py-3.5 transition-all duration-200 ${form.paymentMethod === "lypay" ? "border-[var(--gold)] bg-[var(--gold)]/15 shadow-[0_0_12px_rgba(197,165,90,0.15)]" : "border-[var(--gold)]/15 hover:border-[var(--gold)]/30 hover:bg-[var(--gold)]/5"}`}
                  >
                    <input
                      type="radio"
                      name="payment"
                      value="lypay"
                      checked={form.paymentMethod === "lypay"}
                      onChange={(e) =>
                        setForm({ ...form, paymentMethod: e.target.value })
                      }
                      className="sr-only"
                    />
                    <div
                      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-all ${form.paymentMethod === "lypay" ? "border-[var(--gold)] bg-[var(--gold)]" : "border-[var(--gold)]/30"}`}
                    >
                      {form.paymentMethod === "lypay" && (
                        <div className="h-2 w-2 rounded-full bg-[var(--royal-red-dark)]" />
                      )}
                    </div>
                    <div className="flex flex-1 items-center gap-3">
                  ...
                  <label
                    className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 px-4 py-3.5 transition-all duration-200 ${form.paymentMethod === "cash" ? "border-[var(--gold)] bg-[var(--gold)]/15 shadow-[0_0_12px_rgba(197,165,90,0.15)]" : "border-[var(--gold)]/15 hover:border-[var(--gold)]/30 hover:bg-[var(--gold)]/5"}`}
                  >
                    <input
                      type="radio"
                      name="payment"
                    ...
                    />
                    <div
                      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-all ${form.paymentMethod === "cash" ? "border-[var(--gold)] bg-[var(--gold)]" : "border-[var(--gold)]/30"}`}
                    >
                ...
                {!form.paymentMethod && (
                  <p className="mt-2 text-xs text-red-400/70">
                    يرجى اختيار طريقة الدفع لإتمام الطلب
                  </p>
                )}
              </div>
              ...
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span className="text-sm text-[var(--gold)]/70">
                  أؤكد أن جميع البيانات صحيحة وأوافق على إتمام الطلب
                </span>
              </label>
                      ...
                      try {
                        ...
                        return (
                          <div
                            ...
                            className="p-4 border-2 border-[var(--gold)]/30 rounded-xl m-2 bg-[var(--gold)]/5"
                          >
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <Package className="h-5 w-5 text-[var(--gold)]" />
                                <h4 className="font-bold text-[var(--gold)]">
                                  باقة المناسبات
                                </h4>
                              </div>
                              <div className="flex items-center gap-2">
                            ...
                            {/* عرض محتويات الباقة للقراءة فقط */}
                            <div className="space-y-2 pr-2 border-r-2 border-[var(--gold)]/20">
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <span className="text-[var(--gold)] font-bold">
                                        {(
                                          pkgItem.finalPrice * pkgItem.quantity
                                        ).toFixed(2)}{" "}
                                        د.ل
                                      </span>
                                    </div>
                            ...
                            <div className="mt-3 pt-2 border-t border-[var(--gold)]/20 flex justify-between">
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1">
                          <h4 className="font-semibold text-[var(--cream)]">
                            {item.name}
                          </h4>
