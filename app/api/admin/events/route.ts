...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!name || !price || !category) {
      return NextResponse.json(
        { error: "الاسم والسعر والتصنيف مطلوبة" },
        { status: 400 },
      );
    }

    const result = await sql`
      INSERT INTO events (name, description, price, category, image_url, is_available, is_featured, sort_order, icon)
      VALUES (
        ${name.trim()},
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const newDesc =
      "description" in body ? body.description?.trim() || null : e.description;
    const newPrice =
      "price" in body && body.price != null ? Number(body.price) : e.price;
    const newCat =
      "category" in body ? body.category?.trim() || e.category : e.category;
    const newImg = "image_url" in body ? body.image_url || null : e.image_url;
    const newAvail =
      "is_available" in body ? body.is_available : e.is_available;
    ...
    const newIcon = "icon" in body ? body.icon || null : e.icon;
    const result = await sql`
      UPDATE events SET
        name = ${newName},
        description = ${newDesc},
