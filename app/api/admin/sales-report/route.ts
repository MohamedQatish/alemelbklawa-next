...
export async function GET(request: Request) {
  ...
  // الحالات المكتملة فقط (نفس المستخدمة في صفحة الإحصائيات)
  ...
  try {
    ...
    if (period === "daily") {
      summary = await sql`
        SELECT
          COUNT(CASE WHEN status = ANY(${completedStatuses}) THEN 1 END) as total_orders,
          COALESCE(SUM(CASE WHEN status = ANY(${completedStatuses}) THEN total_amount END), 0) as total_revenue,
          COALESCE(AVG(CASE WHEN status = ANY(${completedStatuses}) THEN total_amount END), 0) as avg_order,
          COUNT(CASE WHEN status = ANY(${completedStatuses}) THEN 1 END) as completed_orders,
      ...
      breakdown = await sql`
        ...
        FROM orders WHERE created_at::date = CURRENT_DATE AND status = ANY(${completedStatuses})
      ...
      topProducts = await sql`
        ...
        WHERE o.created_at::date = CURRENT_DATE AND o.status = ANY(${completedStatuses})
      ...
      topProducts = await sql`
        SELECT oi.product_name, SUM(oi.quantity) as total_qty,
          SUM(oi.quantity * oi.unit_price) as total_revenue
        FROM order_items oi JOIN orders o ON o.id = oi.order_id
        WHERE DATE_TRUNC('month', o.created_at) = DATE_TRUNC('month', CURRENT_DATE) AND o.status = ANY(${completedStatuses})
        GROUP BY oi.product_name ORDER BY total_qty DESC LIMIT 10
      `
      paymentBreakdown = await sql`
        SELECT payment_method, COUNT(*) as count, COALESCE(SUM(total_amount), 0) as revenue
      ...
      breakdown = await sql`
        SELECT TO_CHAR(DATE_TRUNC('month', created_at), 'YYYY-MM') as time_label, COUNT(*) as order_count,
          COALESCE(SUM(total_amount), 0) as revenue
        FROM orders WHERE DATE_TRUNC('year', created_at) = DATE_TRUNC('year', CURRENT_DATE) AND status = ANY(${completedStatuses})
      ...
      topProducts = await sql`
        SELECT oi.product_name, SUM(oi.quantity) as total_qty,
          SUM(oi.quantity * oi.unit_price) as total_revenue
        FROM order_items oi JOIN orders o ON o.id = oi.order_id
        WHERE DATE_TRUNC('year', o.created_at) = DATE_TRUNC('year', CURRENT_DATE) AND o.status = ANY(${completedStatuses})
        GROUP BY oi.product_name ORDER BY total_qty DESC LIMIT 10
      `
      paymentBreakdown = await sql`
        SELECT payment_method, COUNT(*) as count, COALESCE(SUM(total_amount), 0) as revenue
